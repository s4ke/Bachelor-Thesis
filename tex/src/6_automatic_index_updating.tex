...
\\\\
As this seems to be the only reasonable solution for a synchronous update system, we use it in Hibernate Search GenericJPA.
\\\\
...
While we described two different versions of a possible synchronous approach, for the asynchronous version we only have one possible solution available: A trigger based system.
\\\\
...
We can see that both event systems can be useful in different cases. This is the reason we use both systems in Hibernate Search GenericJPA. The following table summarizes the pros and cons once more:
...
In this chapter we described how the challenges explained in \ref{Challenges} are solved in Hibernate Search GenericJPA. We built a standalone integration of hibernate-search-engine, then integrated it with JPA and also described how our automatic index updating mechanism works.
\\\\
While some of the APIs may be not final and not completely optimal (especially the work-intensive trigger update approach), they are a big step in the right direction of making Hibernate Search GenericJPA as developer-friendly as possible. All of the shortcomings in the current API as it can be found on GitHub \footnote{Hibernate Search GenericJPA GitHub repository, see~\cite{hibernate_genericjpa_github}} are a rather small price to pay compared to the usefulness of having Hibernate Search capabilities without needing Hibernate Search ORM.
