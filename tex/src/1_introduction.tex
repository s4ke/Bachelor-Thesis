...
\\\\
When investigating Hibernate Search's project structure \cite{source-code-git}, we can see that the only module apart from some server-integration modules that depends on any ORM logic is "hibernate-search-orm". The modules that contain the indexing engine, the replication logic, alternative backends, etc. are completely independent from any ORM logic. This means, that we could reuse most of the codebase for a generic version of Hibernate Search.
\\\\
This would be a better approach to building a search API for JPA than rewriting everything from scratch. Hibernate Search's engine could then act as the standard for fulltext search in the JPA world instead of having a competing API that would just do the same thing in a different style.
\\\\
\begin{figure}[ht]
	\centering
...
\\
This is why in this thesis we will show how such a generic version can be built. We will look at how Hibernate Search's engine can be reused. Then, we will write a standalone version of this engine and finally integrate it with generic JPA.
