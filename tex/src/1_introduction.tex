...
\newpage
\noindent
Creating this would be a better approach for a search API on top of JPA than rewriting everything from scratch. Hibernate Search could then act as the standard for fulltext search in the JPA world instead of having a competing API that would just do the same thing in a different style.
\\\\
\begin{figure}[ht]
	\centering
