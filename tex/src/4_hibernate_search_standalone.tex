...
With our standalone version, basic index control becomes more streamlined as we don't have to work with the Worker pattern from the SearchIntegrator anymore.
\\
\lstset{language=java}
...
StandaloneSearchFactory searchFactory = ...;
EntityProvider entityProvider = new BasicEntityProvider();
...
-> our aim is to be as compatible with the original. -> same interfaces in the same packages, but slight differences
